...
use crate::gameobjects::empty::Empty;
...
pub fn generate_random_weapon() -> ItemsTypes {
    let weapons = db_manager_ref()
        .lock()
        .unwrap()
        .search("Select * from weapons;");
    ...
    ItemsTypes::WeaponItem(WeaponItem::new(
        weapon.get::<_, String>(1),
        weapon.get::<_, String>(2),
        Raritys::from(weapon.get::<_, String>(3)),
        weapon.get::<_, i32>(4) as u8,
        weapon.get::<_, i32>(5) as u8,
    ))
}

pub fn generate_random_treaure() -> ItemsTypes {
    let treasures = db_manager_ref()
        .lock()
        .unwrap()
        .search("Select * from treasures;");

    let random_number = rand::rng().random_range(0..=treasures.len() - 1);

    let treasure = treasures.get(random_number).unwrap();

    ItemsTypes::TreasureItem(TreasureItem::new(
        treasure.get::<_, String>(1),
        Equipmintslots::None,
        treasure.get::<_, String>(2),
        treasure.get::<_, String>(3),
        Raritys::from(treasure.get::<_, String>(4)),
        treasure.get::<_, i32>(5) as u8,
    ))
}

pub fn generate_random_equip() -> ItemsTypes {
    let equips = db_manager_ref()
        .lock()
        .unwrap()
        .search("Select * from equip;");
    ...
    ItemsTypes::EquipItem(EquipItem::new(
        eq.get::<_, String>(1),
        eq.get::<_, String>(2),
        Equipmintslots::from_string(eq.get::<_, String>(3)),
        eq.get::<_, i32>(5) as u8,
        Raritys::from(eq.get::<_, String>(4)),
        eq.get::<_, i32>(6) as u8,
    ))
}

pub fn generate_random_consumable() -> ItemsTypes {
    let consumables = db_manager_ref()
        .lock()
        .unwrap()
        .search("Select * from consumables");
    ...
    ItemsTypes::ConsumableItem(Consumable::new(
        con.get::<_, String>(1),
        con.get::<_, String>(2),
        Raritys::from(con.get::<_, String>(3)),
        con.get::<_, i32>(4) as u8,
        con.get::<_, i32>(5) as u8,
        con.get::<_, i32>(6) as u8,
    ))
}

pub fn generate_random_empty_room() -> EncounterTypes {
    let rooms = db_manager_ref()
        .lock()
        .unwrap()
        .search("Select * from rooms");
    let random_number = rand::rng().random_range(0..=rooms.len() - 1);

    let room = rooms.get(random_number).unwrap();

    EncounterTypes::Empty(Empty::new(
        room.get::<_, String>(1),
        room.get::<_, String>(2),
    ))
}
